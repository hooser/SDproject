...
export default class DigitalMap extends React.Component{
    ...
    initPieData = () => {
        axios.get({
            ...
            console.log(data);
            let dataPie = [];
            for (let type in data.industrial_type_count) {
                dataPie.push({
                    value: data.industrial_type_count[type],
                    name: type
                });
            }
    ...
    initBarData = () => {
        axios.get({
            ...
            for (let type in data.industrial_type_sum) {
                dataBar.key.unshift(type);
                dataBar.value.unshift((data.industrial_type_sum[type] * 1.0 / 1000).toFixed(2));
            }
    ...
    getBarOption = () => {
        let option = {
            ...
            yAxis: {
                type: 'category',
                data: this.state.dataBar.key.slice(0, 10)
            },
            series: [
                {
                    name: '利润',
                    type: 'bar',
                    data: this.state.dataBar.value.slice(0, 10)
                }
            ]
        };
        return option;
    ...
    buildVecCompanyLayer = (params) => {
        console.log("buildVecCompanyLayer starts!");
        ...
        axios.get({
            ...
            if(data.features) {
                ...
                this.cluterLayer = this.buildCluterLayer(companySource);
            }
        }).catch(function (error) {
            //console.log(error);
        });
    ...
    renderOlMap = () => {
        ...
        //公司图层
        this.companyLayer = this.buildVecCompanyLayer({province: '河北省'});
        ...
        this.map = new OlMap({
            ...
            layers: [osmLayer, this.companyLayer]
            //layers: [osmLayer]
        });
    };
    ...
    render(){
        ...
        return (
            <Row style={{backgroundColor:"white"}}>
                ...
                <Col span={8}>
                    <Pie option={this.getPieOption()} title={pieTitle}/>
                    <Bar option={this.getBarOption()} preTitle={preTitle}  barTitles={this.state.barTitles} updateOption={this.updateBarOption}/>
                </Col>
            </Row>
        );
    }
