...
export default class ListedMapBD extends React.Component{
    ...
    queryCategory = () => {
        console.log("yeilp!");
        console.log(this.state.currnetProvince);
        let currentCompany = '';
        if(this.state.companyType == "上市企业")
        {
            currentCompany = "listed_company";
        }
        else if(this.state.companyType == "中国500强"){
            currentCompany = "china_top_500";
        }
        ...
        axios.get({
            ...
            if(data) {
                console.log(data.result.entity_name);
    ...
    getChildrencurrentProvince = (result,currnetP) => {
        this.setState({currnetProvince:currnetP});
        console.log(this.state.currnetProvince);
    };
...
class QueryCompanyForm extends React.Component{
    ...
    provinceData = ["上海","云南省","内蒙古自治区","北京","吉林省","四川省","天津","宁夏回族自治区","安徽省","山东省","山西省","广东省","广西壮族自治区","新疆维吾尔自治区","江苏省","江西省","河北省","河南省","浙江省","湖北省","海南省","湖南省","甘肃省","福建省","西藏自治区","贵州省","辽宁省","重庆","陕西省","青海省","黑龙江省"];
    cityData = {
        "上海": [
            "上海城区"
        ],
